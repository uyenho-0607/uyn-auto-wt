...
def pytest_runtest_setup(item: pytest.Item):
    ...
    if item.get_closest_marker("general"):
        account = "General"
...
def pytest_runtest_makereport(item, call):
    ...
    if report.when == "call":
        ...
        if Config.get_value("platform") == "web":
            driver = DriverList.all_drivers["web"]
            if driver and driver._is_remote:
                try:
                    capture_and_attach_video(driver)
                    logger.info("Video recording attached to Allure report for failed test")

                except Exception as e:
                    logger.error(f"Failed to attach video recording: {str(e)}")
