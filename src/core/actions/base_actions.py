...
class BaseActions:
    ...
    def _find_elements(
            self, locator: tuple[str, str], timeout=EXPLICIT_WAIT, retries=1, cond=EC.visibility_of_all_elements_located
    ) -> list[WebElement]:
        """Find multiple elements with retry mechanism. Returns empty list if none found."""
        try:
            return self._find_element(locator, timeout, retries, cond)
