...
def capture_and_attach_video(driver):
    selenium_grid_s3_bucket_url = "https://selenium-grid-videos.aquariux.dev"
    video_url = f'<a href="{selenium_grid_s3_bucket_url}/videos/{driver.session_id}.mp4">Session Video</a>'
    allure.attach(video_url, name="Screen Recording Link", attachment_type=allure.attachment_type.HTML)
...
def _cleanup_and_customize_report(data: Dict[str, Any]) -> None:
    ...
    if data.get("attachments"):
        data["attachments"] = [item for item in data["attachments"] if item["name"] == "Screen Recording Link"]
...
def clean_allure_log_files():
    ...
    """
    allure_results_dir = ROOTDIR / 'allure-results'
